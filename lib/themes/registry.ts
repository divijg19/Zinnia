...
/**
 * LeetCode-specific override properties for themes with explicit LeetCode designs.
 */
export interface LeetCodeThemeProperties {
...
export interface ThemeColors {
	...
	fire?: ColorToken;
	dates?: ColorToken;
	// LeetCode: raw `--bg-0..3` / `--text-0..3` / `--color-0..3` palette values
	// (strings, may reference widget-specific `url(#...)` gradients) and raw CSS.
	palette?: { bg?: string[]; text?: string[]; color?: string[] };
	css?: string;
}
...
export interface ThemeDefinition {
	...
	leetcode?: Partial<LeetCodeThemeProperties>;
}
