...
export async function loadTrophyModule(): Promise<any> {
    const preferSrc = (process.env.TROPHY_PREFER_SRC === "1") || (process.env.NODE_ENV !== "production");
    const isTest =
        typeof process !== "undefined" &&
        (process.env.VITEST === "1" || process.env.NODE_ENV === "test" || typeof (globalThis as any).vi !== "undefined");
