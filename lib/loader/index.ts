        ...
        try {
            ...
            for (const trial of toTry) {
                try {
                    ...
                    const leadingSlashDrive = normalized.replace(/^[/\\]([A-Za-z]:[/\\].*)$/, "$1");
                    if (leadingSlashDrive !== normalized && fs.existsSync(leadingSlashDrive)) {
                        if (process.env.LOADER_DEBUG === "1")
                            console.debug("loader: selected renderer spec =>", leadingSlashDrive);
