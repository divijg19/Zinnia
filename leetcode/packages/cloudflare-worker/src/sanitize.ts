...
export function sanitize(config: Record<string, string>): Config {
	...
	if (config.border) {
		...
		sanitized.extensions.push(
			() =>
				(
					generator: { config: { width: number; height: number } },
					_data: unknown,
					_body: unknown,
					styles: string[],
				) => {
					styles.push(
						`#background{stroke-width:${size};width:${generator.config.width - size}px;height:${generator.config.height - size}px;transform:translate(${size / 2}px,${size / 2}px)}`,
					);
				},
		);
	}
