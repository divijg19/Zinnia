...
export async function ContestExtension(
	...
	return async function Contest(generator, _data, body) {
		...
		if (result) {
			...
			const end_time =
				result.history[result.history.length - 1].contest.startTime;
			...
			const extension = new Item("g", {
				...
				children: [
					...
					new Item("text", {
						content:
							result.ranking.globalRanking +
							" / " +
							result.ranking.totalParticipants,
