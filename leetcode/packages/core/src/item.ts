...
export class Item implements Base {
	...
	public stringify(): string {
		...
		const attr = Object.entries(this.attr)
			.map(
				([key, value]) =>
					`${key}="${escapeHtml(Array.isArray(value) ? value.join(" ") : value.toString())}"`,
			)
			.join(" ");
		const children =
		...
		return this.single
			? `<${this.type} ${attr} />`
			: `<${this.type} ${attr}>${this.content ? escapeHtml(this.content) : ""}${children}</${this.type}>`;
	}
	...
	public css(): string {
		...
		return `#${this.attr.id}{${Object.entries(this.style)
			...
			.join(
				";",
			)}} ${this.children?.map((child) => child.css()).join("") || ""}`;
	}
}
