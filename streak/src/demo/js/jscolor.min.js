((global, factory) => {
	...
	var jscolor = (() => {
		var jsc = {
			...
			installBySelector: (selector, rootNode) => {
			...
			getInstances: () => {
				...
				for (var i = 0; i < jsc.instances.length; i += 1) {
					if (jsc.instances[i]?.targetElement) {
			...
			setData: () => {
				...
				if (arguments.length === 3) {
					var data = Object.hasOwn(obj, jsc.dataProp)
						? obj[jsc.dataProp]
						: (obj[jsc.dataProp] = {});
			...
			removeData: () => {
				var obj = arguments[0];
				if (!Object.hasOwn(obj, jsc.dataProp)) {
					return true;
				}
			...
			getData: (obj, prop, setDefault) => {
				...
				if (!Object.hasOwn(data, prop) && setDefault !== undefined) {
			...
			setDataAttr: (el, name, value) => {
				var attrName = `data-${name}`;
				el.setAttribute(attrName, value);
			},
			_attachedGroupEvents: {},
			attachGroupEvent: (groupName, el, evnt, func) => {
				if (!Object.hasOwn(jsc._attachedGroupEvents, groupName)) {
			...
			triggerEvent: (el, eventName, bubbles, cancelable) => {
				if (!el) {
					return;
				}
				var ev = null;
			...
			triggerInputEvent: (el, eventName, bubbles, cancelable) => {
				if (!el) {
					return;
				}
			...
			setStyle: (elm, styles, important, reversible) => {
				...
				for (var prop in styles) {
					if (Object.hasOwn(styles, prop)) {
						...
						if (styles[prop] === null) {
							...
							if (origStyle && Object.hasOwn(origStyle, prop)) {
							...
							if (reversible) {
								...
								if (!Object.hasOwn(origStyle, prop)) {
			...
			getElementSize: (e) => [e.offsetWidth, e.offsetHeight],
			getAbsPointerPos: (e) => {
				var x = 0,
					y = 0;
			...
			containsTranparentColor: (colors) => {
			...
			onControlPointerStart: (e, target, controlName, pointerType) => {
				...
				var registerDragEvents = (doc, offset) => {
			...
			onDocumentPointerMove:
				(_e, target, controlName, _pointerType, offset) => (e) => {
					var thisObj = jsc.getData(target, "instance");
					switch (controlName) {
						case "pad":
			...
			onDocumentPointerEnd:
				(_e, target, _controlName, _pointerType) => (_e) => {
					var thisObj = jsc.getData(target, "instance");
			...
			createASliderGradient: () => {
				...
				var canvas = jsc.createEl("canvas");
				var ctx = canvas.getContext("2d");
				var drawFunc = (width, height, color) => {
					canvas.width = width;
					canvas.height = height;
					ctx.clearRect(0, 0, canvas.width, canvas.height);
			...
			pub: function (targetElement, opts) {
				...
				this.channel = function (name, value) {
					...
					if (value === undefined) {
						if (!Object.hasOwn(this.channels, name.toLowerCase())) {
				...
				function setOption(option, value) {
					...
					if (Object.hasOwn(jsc.enumOpts, option)) {
					...
					if (Object.hasOwn(jsc.deprecatedOpts, option)) {
						var oldOpt = option;
				...
				function drawPicker() {
					...
					p.crossBY.style.height =
						p.crossBX.style.width = `${crossOuterSize}px`;
					...
					p.crossLY.style.width =
						p.crossLX.style.height = `${THIS.pointerThickness}px`;
					p.crossLY.style.left = p.crossLX.style.top =
						Math.floor(crossOuterSize / 2) -
						Math.floor(THIS.pointerThickness / 2) +
						"px";
					p.crossLY.style.top =
						p.crossLX.style.left = `${THIS.pointerBorderWidth}px`;
					...
					p.sldPtrIB.style.border =
						p.sldPtrOB.style.border = `${THIS.pointerBorderWidth}px solid ${THIS.pointerBorderColor}`;
					...
					p.asldPtrIB.style.border =
						p.asldPtrOB.style.border = `${THIS.pointerBorderWidth}px solid ${THIS.pointerBorderColor}`;
				...
				if (jsc.pub.options) {
					for (var opt in jsc.pub.options) {
						if (Object.hasOwn(jsc.pub.options, opt)) {
							try {
				...
				for (var i = presetsArr.length - 1; i >= 0; i -= 1) {
					...
					if (!Object.hasOwn(jsc.pub.presets, pres)) {
						console.warn("Unknown preset: %s", pres);
						continue;
					}
					for (var opt in jsc.pub.presets[pres]) {
						if (Object.hasOwn(jsc.pub.presets[pres], opt)) {
							try {
