...
const preview = {
	...
	update() {
		...
		params.hide_total_contributions = String(
			!params.sections.includes("total"),
		);
		...
		const demoImageURL = `/api/streak/preview?${query}`;
		// update preview
		if (params.type !== "json") {
			...
			document.querySelector(".copy-html").parentElement.style.display =
				"block";
			...
			fetch(demoImageURL)
				...
				.then(
					(data) =>
					(document.querySelector(".output .json pre").innerText =
			...
			document.querySelector(".copy-json").parentElement.style.display =
				"block";
		}
	...
	addProperty(property, value = "#EB5454FF") {
		...
		if (!selectElement.disabled) {
			...
			if (
				...
				document.querySelector("#background-type-gradient").checked
			) {
	...
	exportPhp() {
		...
		const mappings = Object.keys(params)
			.map((key) => {
				const value = params[key].includes(",")
					? params[key]
					: `#${params[key]}`;
				return `  "${key}" => "${value}",`;
			})
	...
	updateCheckboxes(param, selector) {
		...
		[...document.querySelectorAll(`${selector} input[value]`)].forEach(
			(checkbox) => {
...
window.addEventListener(
	...
	() => {
		...
		[...document.querySelectorAll("select:not(#properties)")].forEach(
			(element) => {
		...
		document
			.querySelector("#background-type-solid")
			.addEventListener("change", toggleBackgroundType, false);
		document
			.querySelector("#background-type-gradient")
			.addEventListener("change", toggleBackgroundType, false);
		...
		const updateCheckboxTextField = (parentSelector, inputSelector) => {
			...
			document.querySelector(inputSelector).value = [...checked]
				.map((node) => node.value)
				.join(",");
			preview.update();
		};
		...
		document
			.querySelectorAll(".sections input[type='checkbox']")
			.forEach((el) => {
				el.addEventListener("click", () => {
		...
		document
			.querySelector("#mode")
			.addEventListener("change", toggleExcludedDaysCheckboxes, false);
