...
const createCompactLangNode = ({ lang, x, y, display_format }) => {
	const color =
		/** @type {Record<string,string>} */ (languageColors)[lang.name] ||
		"#858585";
...
const createTextNode = ({
	...
	const cardProgress = hideProgress
		...
		: createProgressNode({
				...
				// name field removed: not part of createProgressNode's public API
				progressBarBackgroundColor,
...
const getStyles = (colors) => {
	/** @type {any} */
	const { _titleColor, titleColor, textColor } = colors || {};
	...
	if (layout === "compact") {
		...
		const compactProgressBar = filteredLanguages
			.map((language) => {
				...
				const languageColor =
					/** @type {Record<string,string>} */ (languageColors)[
						language.name
					] || "#858585";
