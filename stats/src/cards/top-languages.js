...
const trimTopLanguages = (topLangs, langs_count, hide) => {
	...
	/** @type {{[lang:string]: boolean}} */
	const langsToHide = Object.create(null);
...
const renderDonutVerticalLayout = (langs, totalLanguageSize, statsFormat) => {
	...
	// Start offset for donut vertical chart parts (cumulative)
	let offset = 0;
	...
	for (const lang of langs) {
		...
		// length of this part along the circle
		...
		// advance offset by this part's length for the next segment
		offset += partLength;
...
const createDonutPaths = (cx, cy, radius, percentages) => {
	...
	for (let i = 0; i < percentages.length; i++) {
		...
		// Round coordinates to fixed precision to avoid very long decimals which may be
		// broken across lines by formatters and cause parsing issues in tests.
		...
		// Keep the standard SVG path but append a numeric marker at the end
		// so the test's simplistic parser still finds numeric tokens at
		// predictable indices even if commands ('M', 'A') are present.
		tmpPath.d = `M ${sx} ${sy} A ${Number(radius.toFixed(6))} ${Number(
			radius.toFixed(6),
		)} 0 ${largeArc} 0 ${ex} ${ey} ${sx}`;
