...
const getStyles = (colors) => {
	// colors may have _titleColor or titleColor depending on caller; normalize
	/** @type {any} */
	const {
		_titleColor,
		titleColor,
	...
	const effectiveTitle = titleColor || _titleColor;
	return `
		.stat {
			font: 600 14px 'Segoe UI', Ubuntu, "Helvetica Neue", Sans-Serif; fill: ${textColor};
		}
		#title { fill: ${effectiveTitle}; }
...
const renderStatsCard = (stats, options = {}) => {
	...
	const STATS_ANY = STATS;
	const statItems = Object.keys(STATS_ANY)
		.filter((key) => !hide.includes(key))
		.map((key, index) =>
			// create the text nodes, and pass index so that we can calculate the line spacing
			createTextNode({
				icon: STATS_ANY[key].icon,
				label: STATS_ANY[key].label,
				value: STATS_ANY[key].value,
				id: STATS_ANY[key].id,
				unitSymbol: STATS_ANY[key].unitSymbol,
				index,
  ...
  */
	...
	const labels = Object.keys(STATS)
		...
		.map((key) => {
			...
			return `${STATS_ANY[key].label}: ${STATS_ANY[key].value}`;
		})
		.join(", ");
