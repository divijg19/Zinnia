...
describe("Trophy handler ETag & cache behavior", () => {
    ...
    it("serves cached body on upstream 304", async () => {
        ...
        vi.resetModules();
        vi.doMock("../../api/_utils", mockApiUtilsFactory({ readMeta: { body: upstreamBody, etag: "etag-1" } }));
    ...
    it("falls back to cached body on upstream 500", async () => {
        ...
        vi.resetModules();
        vi.doMock("../../api/_utils", mockApiUtilsFactory({ readBody: upstreamBody }));
        (global as any).fetch = vi.fn().mockResolvedValue({ status: 500, headers: { get: () => "image/svg+xml" }, text: async () => "<svg>ERR</svg>" });
