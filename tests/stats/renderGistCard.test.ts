...
describe("test renderGistCard", () => {
    it("should render correctly", () => {
        ...
        expect(document.getElementsByClassName("description")[0].textContent).toContain("Small test repository with different Python programs.");
    ...
    it("should trim header if name is too long", () => {
        document.body.innerHTML = renderGistCard({ ...data, name: "some-really-long-repo-name-for-test-purposes" });
        const [header3] = document.getElementsByClassName("header");
        expect(header3?.textContent).toContain("some-really-long-repo-name-for-test...");
    });
    ...
    it("should trim long description", () => {
        ...
        expect(desc.children[0].textContent).toContain("The quick brown fox jumps over the lazy dog is an");
        expect(desc.children[1].textContent).toContain("English-language pangram—a sentence that contains all");
    });
    ...
    it("should render with all the themes", () => {
        Object.keys(themes).forEach((name) => {
            ...
            const backgroundElement = document.querySelector('[data-testid="card-bg"]');
            const backgroundElementFill = backgroundElement?.getAttribute("fill");
            expect([`#${(themes as any)[name].bg_color}`, "url(#gradient)"]).toContain(backgroundElementFill);
        });
    });
    ...
    it("should not render star count or fork count if either are zero", () => {
        ...
        document.body.innerHTML = renderGistCard({ ...data, starsCount: 1, forksCount: 0 });
    ...
    it("should render without rounding and fallback description", () => {
        ...
        expect(document.getElementsByClassName("description")[0].textContent).toContain("No description provided");
    });
});
