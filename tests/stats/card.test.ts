...
describe("Card", () => {
	...
	it("main-card-body position depends on title visibility", () => {
		...
		expect((main as Element).getAttribute("transform")).toBe(
			"translate(0, 55)",
		);
		...
		expect((main2 as Element).getAttribute("transform")).toBe(
			"translate(0, 25)",
		);
	});
	...
	it("should render with correct colors and gradients", () => {
		...
		const card2 = new Card({
			height: 200,
			colors: { titleColor: t2, textColor: tx2, iconColor: ic2, bgColor: bg2 },
		});
		document.body.innerHTML = card2.render(``);
		...
		expect((gradient as Element).getAttribute("gradientTransform")).toBe(
			"rotate(90)",
		);
