...
export function mockApiUtilsFactory({
    ...
    const readMetaImpl = vi.fn(async () => (readMeta ? readMeta : readBody ? { body: readBody, etag: computeEtag(readBody) } : null));
    ...
    return () => ({
        ...
        ALLOWED_THEMES: new Set(["watchdog", "light", "dark", "onedark", "dracula", "radical", "tokyonight", "merko", "github_dark"]),
        filterThemeParam: (url: URL, key = "theme") => {
            const raw = (url as any).searchParams.get(key);
            if (!raw) return;
            const value = String(raw).trim().toLowerCase();
            if (![...new Set(["watchdog", "light", "dark"])].includes(value) && !value.includes(",")) {
        ...
        setEtagAndMaybeSend304: (reqHeaders: Record<string, unknown>, res: any, body: string) => {
