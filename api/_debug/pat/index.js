export default function handler(_req, res) {
    try {
        ...
        const found = {};
        for (const k of keys) found[k] = process.env[k] ? String(process.env[k]) : undefined;
        const masked = Object.fromEntries(
            Object.entries(found).map(([k, v]) => [k, v ? `${String(v).slice(0, 6)}…${String(v).slice(-4)}` : null])
        );
        try { console.debug("_debug/pat/index: visible keys ->", Object.keys(masked).filter(k => masked[k])); } catch (e) { }
